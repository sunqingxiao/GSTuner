...
class DataSet(object):
    ...
    def next_batch(self, batch_size, shuffle=True):
        ...
        if start + batch_size > self._num_examples:
            ...
            return np.concatenate((features_rest_part, features_new_part), axis=0), np.concatenate((durations_rest_part, durations_new_part), axis=0)
        else:
            self._index_in_epoch += batch_size
            end = self._index_in_epoch
            return self._features[start:end], self._durations[start:end]
