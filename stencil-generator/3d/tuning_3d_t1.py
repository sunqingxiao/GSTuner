...
def FilterParams(paraVector):
    ...
    if streaming and not retime and (blockSize[0]*mergeFactor[0])*(blockSize[1]*mergeFactor[1])*(2*order+1)*8 >= 2 ** maxShmPerBlockLg2:
        return False

    if streaming and retime and blockSize[0]*mergeFactor[0]*blockSize[1]*mergeFactor[1]*order > 2 ** maxShmPerBlockLg2:
        return False
    ...
    for paraVector in filter(FilterParams, itertools.product(
      ...
      [False, True], # prefetching
        ...
        if not streaming:
            ...
            continue
        '''
        if (mf[0] > 1 or mf[1] > 1) and blockMerge == False and retiming == False and prefetching == False:
            group[0].append(paraVector)

        if (mf[0] > 1 or mf[1] > 1) and blockMerge == False and retiming == True and prefetching == False:
            group[1].append(paraVector)
        ...
        if (mf[0] > 1 or mf[1] > 1) and blockMerge == True and retiming == False and prefetching == False:
            group[3].append(paraVector)

        #if (mf[0] > 1 or mf[1] > 1) and blockMerge == True and retiming == True and prefetching == False:
        #    group[8].append(paraVector)
        
        ## 4
        if (mf[0] > 1 or mf[1] > 1) and blockMerge == True and retiming == False and prefetching == True:
